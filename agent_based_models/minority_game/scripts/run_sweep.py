"""Sweep memory length M and produce the money plot: entropy vs efficiency."""
...
from sim.metrics import volatility, efficiency
...
def run_trial(M: int, N: int, n_steps: int, burn_in: int, seed: int, k_max: int = 8):
    """Run one trial and return (alpha, vol, eff, ks_entropy)."""
    ...
    # Use conditional entropy h_diff at largest k as KS estimate
    H_prev = 0.0
    ...
    return config.alpha, vol, eff, ks_entropy
...
def main() -> None:
    ...
    for M in M_values:
        ...
        for s in range(n_seeds):
            seed = 1000 * M + s
            alpha_val, vol, eff, ks_ent = run_trial(M, N, n_steps, burn_in, seed)
    ...
    # Compute means per M for line plots
    ...
    cmap = plt.cm.viridis
    norm = plt.Normalize(vmin=min(all_M), vmax=max(all_M))

    fig, axes = plt.subplots(2, 2, figsize=(13, 10))
    fig.suptitle(
        f"Minority Game – Memory Sweep (N={N}, {n_seeds} seeds/M)",
        fontsize=14,
    )

    # Panel 1: Volatility vs α
    ax = axes[0, 0]
    ...
    # Panel 3: THE MONEY PLOT — Entropy vs Efficiency
    ax = axes[1, 0]
    sc = ax.scatter(all_entropy, all_eff, c=all_M, cmap=cmap, norm=norm,
    ...
    ax.set_title("THE MONEY PLOT: Entropy vs Efficiency")
    ax.grid(True, alpha=0.3)

    # Panel 4: Entropy vs Volatility
    ax = axes[1, 1]
    sc2 = ax.scatter(all_entropy, all_vol, c=all_M, cmap=cmap, norm=norm,
                     alpha=0.5, s=20, edgecolors="none")
