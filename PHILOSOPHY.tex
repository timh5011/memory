...
\section{Project Philosophical Introduction: Memory}
...
\section{Project Deliverable Overview}
...
We need to define metric that capture what progress means for a society and how well the society is doing. We also need to identify different systems with different levels of of "inertia" in a society. For example, for most people I would assume that a democracy and free market economy has more social mobility than a dictatorship.
...
\subsection{Ergodicity and the Birkhoff Ergodic Theorem}
% birkhoff ergodic theorem
Ergodic theory relies on a more general definition than mixing: ergodicity. A measure-preserving dynamical system $(X,\mathcal{B},\mu,T)$ is \textit{ergodic} if:
\[ \text{If } A\in\mathcal{B} \text{ such that } T^{-1}A=A, \text{ then } \mu(A)=0\text{ or } \mu(A)=1. \]

In other words, the phase space of the system cannot be decomposed into smaller subsets which are invariant under the system; that is, there is no region of the phase space which is only ever mapped into itself. This property a similar, but weaker, than that of mixing.
...
%% If the refinement is coarse, this means that points in $X$ ... 
...
\subsection{Lyapunov Exponents and Pesin's Entropy Formula}

% Lyapunov Exponents

While KS entropy contain information about the global complexity of a dynamical system, we can study the system locally with Lyapunov Exponents. These describe how quickly nearby trajectories of the dynamical system diverge or converge.

Here, we assume $(X, \mathcal{B}, \mu, T)$ to be a smooth, measure-preserving dynamical system. Starting with some $x\in X$ as an initial condition, we consider a small perturbation, and let $\delta_0$ be the vector of initial separation between two trajectories. The separation between these trajectories often grows like \[||\delta(t)||\approx e^{\lambda t}||\delta_0||.\]
...
\subsection{Measure-Theoretic Conjugacy of Dynamical Systems}

It is possible that apparently different systems follow the same underlying dynamics. If this is the case, we say that these systems are \emph{conjugate}. Formally, two measure-preserving dynamical systems $(X,\mathcal{B}_X,\mu, T)$ and $(Y,\mathcal{B}_Y,\nu, S)$ are conjugate if there exists a measurable bijection $\phi:X\to Y$ such that
\begin{enumerate}
    \item $\phi$ preserves the measure: $\nu = \phi_*\mu$, where  $\phi_*\mu(A):=\mu(\phi^{-1}(A))$ for all $A\in\mathcal{B}_Y$, and
    \item $\phi$ preserves the dynamics: $\phi\circ T = S\circ\phi$, for $\mu$-almost every $X\in X$. 
\end{enumerate}
...
Given an \emph{alphabet} of finite size $\mathcal{A}=\{1,\dots, n\}$, we call bi-infinite sequences $x:\mathbb{Z}\to\mathcal{A}$, denoted $(x_i)_{i\in\mathbb{Z}}$, \emph{words}. We let $\Omega_n:=\mathcal{A}^\mathbb{Z}$ to be the set of all words. The \emph{Bernoulli product measure}, \( \mu^\mathbb{Z} \), is defined
    \[
    \mu^\mathbb{Z}(x_{i_1} = a_{i_1}, \dots, x_{i_k} = a_{i_k}) = \prod_{j=1}^k \mu(a_{i_j}),
    \]
    with \( \mu(a) \) being the probability assigned to symbol \( a \in A \). The random variables \( \{\pi_i\}_{i \in \mathbb{Z}} \), where \( \pi_i(x) = x_i \), are independent and identically distributed (i.i.d.) with distribution \( \mu \).

We then define the shift map $\sigma:(x)_i=x_{i+1}$. This simply shifts every symbol in the sequence one index to the left.
...
Bernoulli shifts satisfy the Markov property which says that the future state only directly depends on the present state, not on the path which lead to that current state. This is why Markov processes are called memoryless. However, this does not necessarily mean that these processes completely forget the initial conditions, because the probability of reaching that present state in the first place was determined by that past. However, the next step is no longer \emph{directly} dependent on that past. Therefore, these processes tend to forget their history faster. 

\subsection{The Sinai Factor Theorem and the Ornstein Isomorphism Theorem}
...
The Sinai Factor Theorem states that any measure-preserving ergodic dynamical system can be decomposed into a Bernoulli shift of equal entropy to that of the system, and a trivial (invertible) component. An equivalent statement is that if the system has KS entropy $h$, then a Bernoulli shift with KS entropy less than or equal to $h$ is a \emph{factor} of the system. Formally, if $(X,\mathcal{B},\mu,T)$ is measure-preserving ergodic dynamical system, then there exist a measurable, measure-preserving \emph{factor map} 
\[ \pi:X\to\Omega_n, \] such that $\pi\circ T=\sigma\circ\pi$, which projects the chaotic part of the system onto a Bernoulli shift. The trivial part of the system is $\ker\pi$. 
